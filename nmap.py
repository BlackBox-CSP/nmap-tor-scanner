...
class PortScanner(object):
    ...
    def __init__(self):
        ...
        regex = re.compile('Nmap version [0-9]*\.[0-9A-Z]*')
        ...
        for line in self._nmap_last_output.split('\n'):
            if regex.match(line) is not None:
                ...
                regex_version = re.compile('[0-9A-Z]+')
                regex_subversion = re.compile('\.[0-9A-Z]+')
                ...
                if rv is not None and rsv is not None:
                    ...
                    self._nmap_version_number = str(line[rv.start():rv.end()])
                    self._nmap_subversion_number = str(line[rsv.start()+1:rsv.end()])
                break
